...
(AO)
Since model 1 is supposed to be "only explanatory variables of key interest" I would think the best option would be to include variables that only show strong correlation (neg or pos) with crime. When we get to model 2/3 then we can add other factors, multiply variables together, etc. I think police makes sense, both intuitively and from the scatterplot (so long as we show strong leverage/influence) of the 115 county and remove it. 

I also think denisty, one of the probabilities (probsen has strongest negative slightly) would be good to include for this model, on the basis that we're only looking at key explanatory variables that make sense from an implementing policy perspective (stating we should reduce federal wages simply to reduce crime is a hard sell).
...
```
Looking at the leverage we can see that there is one datapoint outside the cooks distance line and are influential to the regression. As identified in the EDA, county 115 has many suspect datapoints. while we cannot determine the specific county from this dataset to better understand why it may be an influential county, we will remove it from the regression. 
...
Our population is all counties in a single state in the United States. The state is unknown to us. We noticed that all of the county IDs are odd. We would need to explore why this is the case. From first impression, this seems to suggest that the data is not a random sample. However, we will move forward assuming this assumption is met.
...
3. No Perfect Collinearity: In the sample (and population), none of the independent variables are constant and there are no exact relationships among the independent variables.
...
If someone suggests that the chart above indicates that the model did not meet the zero-conditional mean assumption, because the line is not perfectly straight at zero, then we could at least say that are OLS estimators are consistent since we have a fairly large n of 89. Consistency means that bias goes to zero due to the large sample size. This is refered to as exogeneity.
...
Examining the residuals versus fitted values plot below, it is difficult to say if heteroskedasticity exists. The band of residuals is fairly uniform until it shrinks for higher fitted values. We see this a bit more clearly with our scale-location plot though it might be argued that this is due to less observations as we go to the right. We ran the Breusch-Pagan test, which assumes the null hypothesis that there is homoskedasticity. The small p-value suggests that there is heteroskedasticity that we need to be aware of, so we will move forward with heteroskedasticity robust standard errors, since this assumption is not met.
...
model2 <- lm(crime ~ density + probsen + police + probconv + probarr  + probconv*probsen, data = crime_transformed_outlier)
plot(model2)
